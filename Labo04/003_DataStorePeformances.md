...
- For each performance test, write the command used to generate the attack with vegeta and copy the file generated by the vegeta plot command.
  ...
  ```
  //TODO command used
  vegeta attack -targets=requests.txt -rate=100 -duration=30s | vegeta report
  ```

  ```
  //TODO copy/link file generated by vegeta plot command

Requests      [total, rate, throughput]         3000, 100.03, 92.60
Duration      [total, attack, wait]             32.396s, 29.99s, 2.406s
Latencies     [min, mean, 50, 90, 95, 99, max]  39.354ms, 480.493ms, 67.755ms, 1.031s, 4.198s, 4.732s, 5.105s
...
- Let's suppose you become suspicious that the algorithm for the automatic scaling of
  ...
  ```
  //TODO
We could check requests from the same IP, or same requests url.
